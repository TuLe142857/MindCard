...
const MOCK_STUDY_CARDS: Card[] = [
  ...
  },
  {
    ...
    back: { text: 'A closure is the combination of a function bundled together with references to its surrounding state (the lexical environment).' },
  },
  {
    id: 3,
    type: 'BASIC',
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex-1 max-w-xl">
            <div className="flex justify-between text-xs font-medium text-slate-400 mb-2">
              <span>Card {currentIndex + 1} of {MOCK_STUDY_CARDS.length}</span>
        ...
        {!isFlipped ? (
          ...
          <div className="flex flex-col animate-in slide-in-from-bottom-4 fade-in duration-300">
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
              <button onClick={() => handleRate(0)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 transition-all">
                <span className="text-xl font-bold mb-1">0</span>
                <span className="text-[10px] uppercase font-semibold">Again</span>
              </button>
              
              <button onClick={() => handleRate(1)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/30 text-orange-400 transition-all">
                <span className="text-xl font-bold mb-1">1</span>
                <span className="text-[10px] uppercase font-semibold">Hard</span>
              </button>
              
              <button onClick={() => handleRate(2)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 text-yellow-400 transition-all">
                <span className="text-xl font-bold mb-1">2</span>
                <span className="text-[10px] uppercase font-semibold">Good</span>
              </button>
              
              <button onClick={() => handleRate(3)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 transition-all">
                <span className="text-xl font-bold mb-1">3</span>
                <span className="text-[10px] uppercase font-semibold">Easy</span>
              </button>
              
              <button onClick={() => handleRate(4)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 transition-all">
                <span className="text-xl font-bold mb-1">4</span>
                <span className="text-[10px] uppercase font-semibold">V. Easy</span>
              </button>
              
              <button onClick={() => handleRate(5)} className="flex flex-col items-center justify-center p-3 rounded-xl bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 transition-all">
